...
{
	float frist_bit = instruction/100;
	return (int) frist_bit;
}

int where(int instruction, int frist_bit)
{
	for (int i = 0; i < frist_bit; i++)
		instruction -= 100;
	return instruction;
}
...
{
	*codigoOperacao = command(instruction);
	*operando = where(instruction, *codigoOperacao);
}
